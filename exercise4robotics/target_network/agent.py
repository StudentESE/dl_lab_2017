...
class Agent():
    ...
    def __init__(self):
        ...
        self.Q_next =  self.forward_pass(self.states_next)

        best_actions_next = tf.one_hot(tf.argmax(self.Q_next, axis=1), 5)
        ...
        # Define the update rules for the target network
        self.rules = self.target_network_update_rules()

    def forward_pass(self, states):
        """
        Defines structure of network, returns Q vector.
        """
    ...
    def target_network_update_rules(self, tau = 0.01):
        tf_vars = tf.trainable_variables()
        num_vars = len(tf_vars)
        rules = []
        for idx, var in enumerate(tf_vars[0:num_vars//2]):
            rules.append(tf_vars[idx + num_vars // 2].assign((var.value() * tau) + ((1 - tau) * tf_vars[idx + num_vars // 2].value())))
        return rules
    ...
    def clear_hist(self):
        """
        Clears the history if new episode starts.
        """
        self.history[:] = 0
    ...
    def make_epsilon_greedy_action(self, sess, obs, epsilon=0.5):
    ...
    def make_super_greedy_action(self, sess, obs):
        """
        Makes greedy action but never "null" step (i.e does not stay in place)
        """
        q_ = self.compute_q(sess, obs)
